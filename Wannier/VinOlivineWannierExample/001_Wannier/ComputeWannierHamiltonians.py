...
hr_filename = 'Mg2SiO4.V_1_hr.dat'
...
for g in G:
    ...
    for i in range(len(E)):
        ...
        OutStr += ')\n'
    _ = plt.hist(np.real(E)-Efermi, orientation='horizontal', bins=100)
    plt.xlabel('Num States')
    plt.ylabel('E$_{Fermi}$ - E')
    plt.title('Eigenvalues')
    plt.savefig(hr_filename + os.path.split(g)[0] + '_Eigenvalues.png')
    ...
    with open(hr_filename + os.path.split(g)[0] + out_suffix, 'w') as outfile:
